...
function generateGearsChainQuestion(seed: number): BaseQuestion {
  ...
  const { options, correctAnswer } = shuffleOptions(
    ...
    [
      oppositeDirText,
      'Roda gigi tidak akan berputar karena terkunci',
      'Roda gigi berputar bolak-balik (osilasi)'
    ],
    seed * 7 + 3
  );
...
function generateGearSpeedQuestion(seed: number): BaseQuestion {
  ...
  const teethB = [30, 36, 45, 60][(seed + 1) % 4];
  ...
  if (isAskB) {
    ...
    const shuffled = shuffleOptions(
      `${rpmB} RPM`,
      [
        `${distractors[0] || rpmB + 20} RPM`,
        `${distractors[1] || rpmB + 40} RPM`,
        `${rpmA} RPM`
      ],
      seed * 11 + 5
    );
    ...
    const shuffled = shuffleOptions(
      'Roda gigi A berputar lebih cepat daripada roda gigi B',
      [
        'Roda gigi B berputar lebih cepat daripada roda gigi A',
        'Kedua roda gigi berputar dengan kecepatan putar (RPM) yang persis sama',
        'Roda gigi B memiliki torsi lebih kecil daripada roda gigi A'
      ],
      seed * 13 + 7
    );
    options = shuffled.options;
...
function generateBeltQuestion(seed: number): BaseQuestion {
  ...
  const { options, correctAnswer } = shuffleOptions(
    ...
    [
      wrongDirText,
      'Puli B tidak berputar karena terjadi selip total',
      'Puli B berputar bolak-balik'
    ],
    seed * 5 + 11
  );
...
function generatePulleyQuestion(seed: number): BaseQuestion {
  ...
  const { options, correctAnswer } = shuffleOptions(
    `${force} kg`,
    [
      `${distractors[0] || force + 30} kg`,
      `${distractors[1] || force * 2} kg`,
      `${distractors[2] || force + 15} kg`
    ],
    seed * 17 + 13
  );
...
function generateLeverQuestion(seed: number): BaseQuestion {
  ...
  if (isCalc) {
    ...
    const armWeight = [1, 2, 0.5][(seed + 1) % 3];
    const armForce = [2, 3, 4, 5][(seed + 2) % 4];
    ...
    const { options, correctAnswer } = shuffleOptions(
      ...
      [
        positions[1],
        positions[2],
        'Semua posisi membutuhkan tenaga yang sama persis'
      ],
      seed * 23 + 19
    );
...
function generateHydraulicQuestion(seed: number): BaseQuestion {
  const area1 = [2, 5, 10][seed % 3];
  const ratio = [5, 10, 20, 50][(seed + 1) % 4];
  const area2 = area1 * ratio;
  const force1 = [10, 20, 30, 50][(seed + 2) % 4];
  const force2 = force1 * ratio;

  const { options, correctAnswer } = shuffleOptions(
    `${force2} kg`,
    [
      `${force1} kg`,
      `${Math.round(force2 / 2)} kg`,
      `${force2 * 2} kg`
    ],
    seed * 29 + 23
  );
...
function generateCircuitQuestion(seed: number): BaseQuestion {
  ...
  const { options, correctAnswer } = shuffleOptions(
    correctText,
    [
      'Semua lampu akan tetap menyala redup',
      'Terjadi korsleting / hubungan singkat',
      'Hanya Lampu L1 yang menyala'
    ],
    seed * 31 + 29
  );
...
function generateStabilityQuestion(seed: number): BaseQuestion {
  ...
  if (isTruck) {
    const { options, correctAnswer } = shuffleOptions(
      'Truk A (Muatan berat ditempatkan di lantai dasar bak)',
      [
        'Truk B (Muatan berat ditempatkan di rak bagian paling atas)',
        'Kedua truk memiliki tingkat risiko terguling yang persis sama',
        'Truk B lebih stabil karena gaya tekan gravitasi lebih tinggi'
      ],
      seed * 37 + 31
    );
    ...
    const { options, correctAnswer } = shuffleOptions(
      'Kotak A (Alas lebar dan tinggi benda rendah)',
      [
        'Kotak B (Alas sempit dan tinggi benda tinggi)',
        'Kotak C (Alas sempit dengan beban berat di puncak)',
        'Semua kotak memiliki kestabilan yang sama'
      ],
      seed * 41 + 37
    );
...
function generateInclineQuestion(seed: number): BaseQuestion {
  const { options, correctAnswer } = shuffleOptions(
    'Papan A (Paling panjang dengan sudut kemiringan paling landai)',
    [
      'Papan B (Panjang sedang dengan sudut kemiringan sedang)',
      'Papan C (Paling pendek dengan sudut kemiringan paling curam)',
      'Semua papan memerlukan gaya dorong yang sama besar'
    ],
    seed * 43 + 41
  );
...
function generateSpringQuestion(seed: number): BaseQuestion {
  ...
  if (isParallel) {
    const { options, correctAnswer } = shuffleOptions(
      'Rangkaian Paralel (Kedua pegas menahan beban berdampingan)',
      [
        'Rangkaian Seri (Kedua pegas disambung ujung ke ujung)',
        'Kedua susunan mengalami penurunan yang sama persis',
        'Tergantung pada bahan pelumas pegas'
      ],
      seed * 47 + 43
    );
...
function generateBimetalQuestion(seed: number): BaseQuestion {
  ...
  const { options, correctAnswer } = shuffleOptions(
    expectedDirection,
    [
      isHeated ? 'Melengkung ke arah Logam A (ke atas)' : 'Melengkung ke arah Logam B (ke bawah)',
      'Tetap lurus sempurna tanpa melengkung',
      'Terputus di bagian sambungan perekat'
    ],
    seed * 59 + 53
  );
...
function generateFrictionQuestion(seed: number): BaseQuestion {
  ...
  if (isLubricated) {
    const { options, correctAnswer } = shuffleOptions(
      'Mengurangi gaya gesek dan mencegah keausan material antar permukaan logam',
      [
        'Meningkatkan daya cengkeram roda gigi',
        'Menambah berat beban mesin agar lebih stabil',
        'Menghantarkan arus listrik statis pada mesin'
      ],
      seed * 61 + 59
    );
    ...
    const { options, correctAnswer } = shuffleOptions(
      'Balok A pada permukaan es licin (koefisien gesek terkecil)',
      [
        'Balok B pada permukaan aspal kasar',
        'Balok C pada permukaan karet bertekstur',
        'Semua balok membutuhkan gaya tarik yang sama besar'
      ],
      seed * 67 + 61
    );
...
function generateBuoyancyQuestion(seed: number): BaseQuestion {
  const { options, correctAnswer } = shuffleOptions(
    'Pada Bejana Air Garam (karena massa jenis air garam lebih tinggi dari air tawar)',
    [
      'Pada Bejana Air Tawar murni',
      'Balok mengapung pada ketinggian yang persis sama di kedua bejana',
      'Balok akan tenggelam sepenuhnya di air garam'
    ],
    seed * 71 + 67
  );
